To deploy these scripts to the ESP watershed, copy both the "scripts" and "shared" folder into the watershed's
  ...
  correct folders.
Next, open the watershed in the CAVI and add the "Download_ESPs" and "Flow_Modifier" scripts to the "Modeling" tab.

The download and route ESP traces scripts should be ran in sequence:
	"Download_ESP"		- downloads data from NWRFC website (it takes about 5 minutes to run)
...
You'll also need to add the 'sqlitejdbc-0.5.4.jar' jar file to your CAVI install under <CAVI install>/CAVI/jar
	OR
set up a dummy import module (see the 'Modifying CWMS Model for ESP Analysis' word doc in cwms_esp/instructions folder)